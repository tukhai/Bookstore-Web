...
def book_details(request, book_id):
    ...
    geo_info = GeoIP2().city('192.206.151.131')
    if not geo_info:
        geo_info = GeoIP2().city("72.14.207.99")
...
def checkout_paypal(request, cart, orders):
    if request.user.is_authenticated():
        ...
        paypalrestsdk.configure({
            "mode": "sandbox",
            "client_id": "AdQLh0epann5L3vRCQRxwjazV835DYZVOZcbWp-KDFq5Ld0lt0WDtmIX_Tk1ANu0qnmzqantYGEnF2b4",
            "client_secret": "EGinKmmPn7svZrInCPqzIhi3ZGzppBC53Rmuziyq4c8ZFiw6MfPX3S2-3AbrvQpHxWSJq9UGkOMYPwqp"
        })
...
def checkout_stripe(cart, orders, token):
    stripe.api_key = "sk_test_cn6XR2TFV9VyGSq0KanZcBin"
    total = 0
    for order in orders:
        total += (order.book.price * order.quantity)
    status = True
    ...
    if request.user.is_authenticated():
        ...
        if processor == 'paypal':
            ...
            if payment.execute({"payer_id": payment.payer.payer_info.payer_id}):
                message = "Succes! Your order has been completed, and is being processed. Payment ID: %s" % (payment.id)
                cart.active = False
            ...
            cart.save()
            message = "Succes! Your order has been completed, and is being processed. Payment ID: %s" % (cart.payment_id)
            context = {
                'message': message
            }
            return render(request, 'store/order_complete.html', context)
